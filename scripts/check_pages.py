...
templates/pages/<type>.md. Fails (exit 1) when a page:
  1. lacks a front-matter field, leaves one empty, or keeps a template <placeholder>
  ...
  4. has a section that is not substantive: fewer than MIN_WORDS words of prose, or, for
     Related, no link, or, for Sources, no [^key] citation
  5. keeps a template guidance comment, or uses provisional language (GR-3.2)
  6. cites a key that is not in content/references.yml or not in its `sources`, lists a
     ...
     review_record that does not exist
...
PROVISIONAL = [
    ...
    r"\bv0\.\d", r"\bunder construction\b", r"\blorem ipsum\b",
...
def check_page(path: Path, meta: dict, body: str, templates: dict, refs: dict, glossary_ids: set,
    ...
    for heading, text in found:
        ...
        elif words(text) < MIN_WORDS:
            problems.append(f"{where}: section '{heading}' has {words(text)} words; at least {MIN_WORDS} are needed (GR-4.1)")
